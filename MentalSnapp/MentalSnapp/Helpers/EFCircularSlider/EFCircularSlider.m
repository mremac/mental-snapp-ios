...
- (void)defaults {
    ...
    _unfilledColor = [[UIColor whiteColor] colorWithAlphaComponent:0.6];
...
- (void)drawRect:(CGRect)rect
{
    ...
    int fillValue = _currentValue/(100/7);
    fillValue = fillValue+1;
    CGFloat fillNewAngle = (MAXANGLE/7)*fillValue;
    fillNewAngle = (MAXANGLE-fillNewAngle)-3;
    ...
    float sectionAngle = (float)MAXANGLE/7;
    for (int r=0; r<7; r++){
        float currentSectionAngle = (sectionAngle * (float)r);
        ...
        CGContextAddArc(ctx,
                        ...
                        self.centerPoint.y,
                        self.radius,
                        [EFCircularSlider toRadian:-90 + currentSectionAngle-.5 withMax:MAXANGLE],
                        [EFCircularSlider toRadian:-90+ currentSectionAngle+.5 withMax:MAXANGLE],
                        0);
...
-(void) drawLabels:(CGContextRef)ctx {
    if(labelsEvenSpacing == nil || [labelsEvenSpacing count] == 0) {
        ...
        for (int i=0; i<[labelsEvenSpacing count]; i++)
        {
            ...
            CGSize labelSize=CGSizeMake(1, 30);
