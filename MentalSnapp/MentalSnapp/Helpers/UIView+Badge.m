...
-(void)setBadgeValue:(NSInteger)value {
    if(value != 0 || self.displayWhenZero) {
        CGSize numberSize = [[NSString stringWithFormat:@"%ld", (long)value] sizeWithAttributes:@{NSFontAttributeName:self.font}];
        ...
        if(self.placement == kBadgePlacementUpperBest) {
            ...
            if(rightMostInWindow.x > [[UIScreen mainScreen] bounds].size.width) {
...
-(void)drawRect:(CGRect)rect {
    if(self.badgeValue != 0 || self.displayWhenZero) {
        ...
        [self.textColor set];
        CGSize numberSize = [[NSString stringWithFormat:@"%ld", (long)self.badgeValue] sizeWithAttributes:@{NSFontAttributeName:self.font}];
        
        NSMutableParagraphStyle* paragraphStyle = [[NSMutableParagraphStyle alloc] init];
        [paragraphStyle setAlignment:NSTextAlignmentCenter];
        [paragraphStyle setLineBreakMode:NSLineBreakByClipping];
        [[NSString stringWithFormat:@"%ld", (long)self.badgeValue] drawInRect:CGRectMake(self.outlineWidth + 3, (rect.size.height / 2.0) - (numberSize.height / 2.0), rect.size.width - (self.outlineWidth * 2) - 6, numberSize.height) withAttributes: @{NSFontAttributeName:self.font, NSParagraphStyleAttributeName:paragraphStyle}];
    }
}
...
-(MLTBadgeView *)badge {
    ...
    if(existingView) {
        if(![existingView isKindOfClass:[MLTBadgeView class]]) {
            NSLog(@"Unexpected view of %@ class found with badge tag.",[MLTBadgeView class]);
            return nil;
        } else {
